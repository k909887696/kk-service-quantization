...
public class ConceptDailyServiceImpl extends MppServiceImpl<ConceptDailyMapper, ConceptDaily> implements IConceptDailyService {
    ...
    @Resource
    public ConceptDailyMapper mapper;
    /**
    * 分批批量插入
    * @param list 数据列表
    * @return
    ...
    * @return 结果集
    */
    public PageResult<ConceptDaily>  getConceptDailyPageResult(BasePage vo){

        QueryWrapper<ConceptDaily> query = new QueryWrapper<>();
        IPage<ConceptDaily> page = new Page<>(vo.getPageIndex(),vo.getPageSize());

        //这里开始编写查询条件

        page = mapper.selectPage(page,query);
        PageResult<ConceptDaily>  pageResult = new PageResult<>();
