...
public class ModuleLogFactory {
    ...
    public static void start(String logModuleName) {
        ...
        TriggeringPolicy tp = SizeBasedTriggeringPolicy.createPolicy("2MB");// 大小分割
       ...
       RollingFileAppender appender = RollingFileAppender.createAppender(String.format( "%s.log",logModuleName,logModuleName)
               ,rootLogDir + File.separator + logModuleName+ File.separator + "${date:yyyy-MM-dd}" + File.separator + logModuleName+"-%d{yyyy-MM-dd-HH}-" + "%i.log"
