...
public class SyUserServiceImpl extends MppServiceImpl<SyUserMapper, SyUser> implements ISyUserService {
    ...
    * @return 结果集
    */
    public PageResult<SyUser>  getPageResult(BasePage vo){

        QueryWrapper<SyUser> query = new QueryWrapper<>();
        IPage<SyUser> page = new Page<>(vo.getPageIndex(),vo.getPageSize());

        //这里开始编写查询条件

        page = mapper.selectPage(page,query);
        PageResult<SyUser>  pageResult = new PageResult<>();
