 ...
 * 概念基本信息任务执行器
 */
@Service("ThsConceptDetailTaskExecutor")
@Slf4j
public class ThsConceptDetailTaskExecutor implements ITaskExecutor {
    ...
    public void executeTask(String params) {
        ...
        for (String id : vo.getIds())
        {
            if(StringUtils.isBlank(id))
                continue;
            vo.setTsCode(id);
            TushareData<ConceptDetail> res =  tushareDataApi.thsMember(vo);
