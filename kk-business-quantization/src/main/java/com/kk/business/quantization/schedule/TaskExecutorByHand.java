package com.kk.business.quantization.schedule;
...
@Slf4j
@Component
public class TaskExecutorByHand {
    @Resource
        ...
        if(list != null && list.size() >0) {//没有需要执行策略
            ...
            for (CollectionPolicy p : list) {
                try {
                    ...
                    collectionPolicyMapper.updateExMsgAndRunCount(p.getPolicyId(), e.getMessage() + "|" + ExceptionUtils.getStackTrace(e));
                }
            }
        }
        log.info("{}|{}"
