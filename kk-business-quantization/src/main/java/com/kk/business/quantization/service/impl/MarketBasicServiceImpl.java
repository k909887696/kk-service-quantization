package com.kk.business.quantization.service.impl;


import org.springframework.stereotype.Service;
import jakarta.annotation.Resource;
import java.util.List;
import java.util.stream.Collectors;
import com.baomidou.mybatisplus.extension.plugins.pagination.Page;
import com.baomidou.mybatisplus.extension.service.impl.ServiceImpl;
import com.kk.business.quantization.dao.entity.MarketBasic;
import com.kk.business.quantization.dao.mapper.MarketBasicMapper;
import com.kk.business.quantization.service.IMarketBasicService;
import com.kk.business.quantization.model.vobase.req.MarketBasicListReqVo;
import com.kk.business.quantization.model.vobase.res.MarketBasicListResVo;
import com.kk.business.quantization.model.vobase.req.MarketBasicAddReqVo;
import com.kk.business.quantization.model.vobase.req.MarketBasicEditReqVo;
import com.kk.business.quantization.model.vobase.res.MarketBasicResVo;
import com.kk.business.quantization.model.vobase.req.MarketBasicDetailsReqVo;
import com.kk.business.quantization.model.vobase.req.MarketBasicDeleteReqVo;
import com.kk.common.base.model.PageResult;
import com.kk.common.utils.BeanUtil;
import com.kk.common.exception.BusinessException;
/**
 * <p>
 * 市场基本信息 服务实现类
 * </p>
 *
 * @author kk
 * @since 2026-06-04
 */
@Service
public class MarketBasicServiceImpl extends ServiceImpl<MarketBasicMapper, MarketBasic> implements IMarketBasicService {


    /**
    * 分批批量插入市场基本信息
    * @param list 数据列表
    * @return
    */
    @Override
    public void insertMarketBasicBatchSomeColumn(List<MarketBasic> list)
    {

        if(list ==null || list.size()<=0) return ;
        int totalCount = list.size();
        int size = 1000;
        int index = 1;
        int totalPage = totalCount / size;
        if( totalPage * size < totalCount ) totalPage += 1;

        for(;index<=totalPage;index++)
        {
            List<MarketBasic> tempList = list.stream().skip((index-1)*size).limit(size).collect(Collectors.toList());
            this.baseMapper.insertBatchSomeColumn(tempList);
        }
    }
    /**
    * 单条插入市场基本信息
    * @param vo 请求参数
    * @return 结果集
    */
    @Override
    public void insertMarketBasic(MarketBasicAddReqVo vo)
    {
        MarketBasic model = new MarketBasic();
        BeanUtil.copyProperties(vo,model);
        this.baseMapper.insert(model);
    }
    /**
    * 更新市场基本信息
    * @param vo 请求参数
    * @return 结果集
    */
    @Override
    public int updateMarketBasic(MarketBasicEditReqVo vo)
    {
        MarketBasic model = new MarketBasic();
        BeanUtil.copyProperties(vo,model);
        int r = this.baseMapper.updateById(model);
        if(r != 1)
        {
            throw new BusinessException("市场基本信息更新失败!");
        }
        return r;
    }
    /**
    * 单条查询市场基本信息
    * @param vo 请求参数
    * @return 结果集
    */
    @Override
    public MarketBasicResVo selectMarketBasicById(MarketBasicDetailsReqVo vo)
    {
        MarketBasic model = new MarketBasic();
        BeanUtil.copyProperties(vo,model);
        MarketBasic res = this.baseMapper.selectById(model);
        MarketBasicResVo resVo = new MarketBasicResVo();
        BeanUtil.copyProperties(res,resVo);
        return resVo;
    }
    /**
    * 删除市场基本信息
    * @param vo 请求参数
    * @return 结果集
    */
    @Override
    public int deleteMarketBasicById(MarketBasicDeleteReqVo vo)
    {
        MarketBasic model = new MarketBasic();
        BeanUtil.copyProperties(vo,model);
        int r = this.baseMapper.deleteById(model);
        if(r != 1)
        {
            throw new BusinessException("市场基本信息删除失败!");
        }
        return r;
    }
    /**
    * 分页获取市场基本信息结果集
    * @param vo 请求参数
    * @return 结果集
    */
    @Override
    public PageResult<MarketBasicListResVo>  selectMarketBasicPageList(MarketBasicListReqVo vo){

        Page<MarketBasicListResVo> page = new Page<>(vo.getPageIndex(),vo.getPageSize());
        Page<MarketBasicListResVo> results = this.baseMapper.selectMarketBasicPageList(page,vo);
        PageResult<MarketBasicListResVo>  pageResult = new PageResult<>();

        return pageResult.convertPage(results);
    }



}
