 ...
 * @since 2023-05-18
 */
@Tag(name = "/"+ServiceSystemKey.ServiceName+"/"+SystemKey.ApiPrefixLogin+"/v1/sy_user",description = "用户信息")
@ResponseBody
@RestController
@RequestMapping("/"+ServiceSystemKey.ServiceName+"/"+SystemKey.ApiPrefixLogin+"/v1/sy_user")
public class SyUserController {
    ...
    @PostMapping("/get_sy_user_page_list")
    public ApiResult< PageResult<SyUserListDto> > getSyUserPageList(@Valid @RequestBody SyUserListVo vo)   {

        return new  ApiResult(syUserService.selectPageList(vo));
    ...
    public ApiResult<?> deleteById(@Valid @RequestBody SyUserDeleteVo vo)   {
        syUserService.deleteById(vo);
    ...
    @PostMapping("/insert")
    public ApiResult<?> insert(@Valid @RequestBody SyUserAddVo vo)   {
        syUserService.insert(vo);
        return ApiResult.SUCCESS;
    ...
    @PostMapping("/update")
    public ApiResult<?> update(@Valid @RequestBody SyUserEditVo vo)   {
        syUserService.update(vo);
        return ApiResult.SUCCESS;
    ...
    public ApiResult< SyUserDto > getDetails(@Valid @RequestBody SyUserDetailsVo vo)   {

        return new  ApiResult(syUserService.selectById(vo));
