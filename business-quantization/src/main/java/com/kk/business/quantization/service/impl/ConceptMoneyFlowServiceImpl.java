...
public class ConceptMoneyFlowServiceImpl extends MppServiceImpl<ConceptMoneyFlowMapper, ConceptMoneyFlow> implements IConceptMoneyFlowService {
    ...
    @Resource
    public ConceptMoneyFlowMapper mapper;
    /**
    * 分批批量插入
    * @param list 数据列表
    * @return
    ...
    * @return 结果集
    */
    public PageResult<ConceptMoneyFlow>  getConceptMoneyFlowPageResult(BasePage vo){

        QueryWrapper<ConceptMoneyFlow> query = new QueryWrapper<>();
        IPage<ConceptMoneyFlow> page = new Page<>(vo.getPageIndex(),vo.getPageSize());

        //这里开始编写查询条件

        page = mapper.selectPage(page,query);
        PageResult<ConceptMoneyFlow>  pageResult = new PageResult<>();
