...
public class IndexBasicServiceImpl extends MppServiceImpl<IndexBasicMapper, IndexBasic> implements IIndexBasicService {
    ...
    @Resource
    public IndexBasicMapper mapper;
    /**
    * 分批批量插入
    * @param list 数据列表
    * @return
    ...
    * @return 结果集
    */
    public PageResult<IndexBasic>  getPageResult(BasePage vo){

        QueryWrapper<IndexBasic> query = new QueryWrapper<>();
        IPage<IndexBasic> page = new Page<>(vo.getPageIndex(),vo.getPageSize());

        //这里开始编写查询条件

        page = mapper.selectPage(page,query);
        PageResult<IndexBasic>  pageResult = new PageResult<>();
